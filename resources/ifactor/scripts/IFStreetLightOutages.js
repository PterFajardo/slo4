...
(function(){
    ...
    var app = angular.module('SLO', ['IFMap','IFMapInfoBox','IFMapAssetSearch','IFMapAlert','IFMapMessageBox','IFMapMenu'])
       ...
       /* .service('appProperties', function(){
            return{
                environment: sloObject
            };
        })*/
        .controller('SloController', ['$scope', '$sce', '$http', '$log', '$location', function($scope, $sce, $http, $log, $location) {
            ...
            sloObject = {
                ...
                serviceHost: $location.host().indexOf('localhost') >= 0 ? $location.protocol() + '://dvlp.streetlightoutages.com:81' : $location.protocol() + '://' + $location.host()
            };
    }]);
